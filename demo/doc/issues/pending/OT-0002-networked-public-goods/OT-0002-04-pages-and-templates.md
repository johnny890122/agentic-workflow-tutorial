...
- **No artifact** — the layout questions belonged to `OT-0002-01`, which was
  blocked; these pages answer them by default rather than by choice.
- **Out of scope.** Session config, bots.

## Acceptance Criteria

- [x] Three pages exist and repeat for all 10 rounds.
- [x] `Decide` shows the current links and the itemised history of each.
...
- The history table has never been reviewed at full load with three partners.
  Under the ring it is two columns; an unequal-degree arrangement widens it.
...
through all ten rounds, history table populating each round, payoffs matching
hand calculation (100 kept + 0 own + 67 + 20 = 187). Rule 10 held — the observed
participant saw players 1 and 2 and never player 3.

`partner_history()` builds the table server-side as one row per completed round
and one column per current partner, so the worst case stays on one screen.
