...
- **Arrangement:** ring, per `artifacts/OT-0002-01-link-arrangements/DECISION.md`.
  Provisional — chosen by the implementer because the artifact stage could not run.
- **Where it runs:** `creating_session(subsession)`, which fires once per round at
  session creation. `REQ-0001` rule 6 makes the links visible before the
...
- **How:** shuffle the four `id_in_group` values into a cycle; each player's
  neighbours are the entries either side of them, stored ascending as
  `Player.links`.
...
- [x] Links are redrawn each round and are independent of the previous round.
...
string. Verified in a live session: one participant was linked to players 1 and 3
in round 1 and players 1 and 2 in round 10, so the redraw is real.

Changing the arrangement later means changing this function only — the payoff
rule and templates already handle a variable number of links.
